...
fn test_new_cc_build() {
    let (flags, exec_ctx) = Flags::parse(&["build".to_owned()]);
    let build = Build::new(Config { ..Config::parse(flags, exec_ctx) });
...
fn test_find_target_with_config() {
    let (flags, exec_ctx) = Flags::parse(&["build".to_owned()]);
    let mut build = Build::new(Config { ..Config::parse(flags, exec_ctx) });
    let target = TargetSelection::from_user("x86_64-unknown-linux-gnu");
